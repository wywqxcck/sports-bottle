...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuehuni != cur->valuehuni)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehuni Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaghuni = 1;
            ret = 1;
        }
    }
    if(last->valuewatere != cur->valuewatere)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuewatere Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagwatere = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flaghuni)
    {
        devStatusTmp.valuehuni = gizY2X(huni_RATIO,  huni_ADDITION, dataPoints->valuehuni); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehuni,huni_LEN);
        byteFieldOffset += huni_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagwatere)
    {
        devStatusTmp.valuewatere = exchangeBytes(gizY2X(watere_RATIO,  watere_ADDITION, dataPoints->valuewatere)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewatere,watere_LEN);
        byteFieldOffset += watere_LEN;
    }
